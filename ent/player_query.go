...
func (pq *PlayerQuery) QueryGame() *GameQuery {
...
func (pq *PlayerQuery) WithGame(opts ...func(*GameQuery)) *PlayerQuery {
	query := &GameQuery{config: pq.config}
...
func (pq *PlayerQuery) sqlAll(ctx context.Context) ([]*Player, error) {
	...
	if query := pq.withGame; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].game_players; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "game_players" returned %v`, n.ID)
			}
			for i := range nodes {
